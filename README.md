...
| `tool-info` *         | Any string. At least one, most tools have three. Place on new line like `- bullet: National and Provincial election results from 2011 and 2014`.                                                                              | Bullet points of what the tool is about, what it can be used for, and other pertinent information                |
| `slideshow` *         | The name and extension of the file (`image.jpg`). Must be at least two. The file itself should be placed in `/img/tools`. Place on new line like `- image: wm1.jpg`.                                                          | Screenshots of the tool that are displayed on the tool’s page                                                    |
| `videos`              | The ID of the video on YouTube. Place on new line like `- id: lXKDBoRSqxo`                                                                                                                                                    | An embedded video on the tool’s page                                                                             |
| `creators`            | The `name` of anyone listed in `_team`. Their `name` is the same as the file name. Place on new line like `- name: adi`                                                                                                       | OpenUp team members who worked on this project                                                                   |
| `external-creators`   | Their full name, file name, and valid url. The image itself should be in `/img/team`. Place on new lines for each, like <br>`- name: Peter Koen` <br> `image: peter.jpg` <br> `external-url: http://pondo.co`                             | Individuals that contributed extensively on the projects who aren’t part of the OpenUp team                      |
| `collaborators`       | The proper name, file name, and valid url. The image itself should be in `/img/collaborators`. Place on new lines for each, like ```- name: Open Society Foundations  image: osf.png  external-url: http://www.osf.org.za/``` | Any collaborating organisation that supported the project                                                        |
...
| `tools`      | Any tool in `_tools`. Can be multiple, can be zero. Place on new line like `- name: wazimap`                                     | The related tool(s)                                                                                                                                                |
| `themes`     | Any theme in `_themes`. Can be multiple, can be zero. Place on new line like `- name: tech`                                      | The related theme(s)                                                                                                                                               |
...
| `biography` * | A string. Place on new line like `- highlight: Can sink shots from way dowtown`                               | A short highlight of that person’s (professional) life. Listed under their photo on their profile page     |
